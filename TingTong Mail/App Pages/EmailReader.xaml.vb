...
Class EmailReader
    ...
    Dim WithEvents Timer As New Forms.Timer
    ...
    End Function
    Private Sub Goback_MouseDown(sender As Object, e As MouseButtonEventArgs) Handles GoBack.MouseDown
        Browser1.Navigate("about:blank")
        EmailHolder.Children.Clear()
        EmailHolder.Height = 1
        Dim parent As Frame = FindParent(Of Frame)(Me)
        parent.NavigationService.RemoveBackEntry()
        Browser1.Visibility = Visibility.Hidden
        Dim sb As New Storyboard
        sb = Me.FindResource("HideMainframe")
        Storyboard.SetTarget(sb, parent)
        AddHandler sb.Completed, AddressOf sb_done
        sb.Begin()
    End Sub
    Friend Sub sb_done()
        Browser1.Visibility = Visibility.Hidden
    ...
    Private Sub LoadIcons()
        ...
        End Try
        Try
            Dim path = New Uri("pack://application:,,,/TingTong Mail;component/icons/close.png")
            Dim Image As BitmapImage = PhotoLoader.LoadImage(path, 30)
            CloseAttachmentDialog.Source = Image
        Catch ex As Exception
        End Try
        Try
    ...
    Private Sub AnimationDone()
        Dim parent = FindParent(Of Frame)(Me)
        parent.Visibility = Visibility.Hidden
        parent.Opacity = 1
    End Sub
